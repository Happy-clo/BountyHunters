...
public class BountyList extends PluginInventory {
    ...
    public Inventory getInventory() {
        ...
        for (int j = min; j < max && j < bounties.size(); j++) {
            ...
            boolean isTarget = bounty.hasTarget(player),
                    isCreator = bounty.hasCreator(player),
                    isHunter = bounty.hasHunter(player),
                    noCreator = !bounty.hasCreator();
            builder.applyConditions(
                    new String[]{"noCreator", "isCreator", "extraCreator", "isExtra", "isTarget", "isHunter", "!isHunter"},
            ...
            builder.applyPlaceholders(
                    ...
                    "target_tax", UtilityMethods.format(BountyHunters.getInstance().getConfigManager().targetSetTax.calculate(bounty.getReward(), false)),
                    "expire_delay", UtilityMethods.formatDelay(bounty.getExpireDelay()));
        ...
        if (BountyHunters.getInstance().getConfig().getBoolean("player-tracking.enabled")) {
            ...
            compassLore.add("");
            compassLore.add(Language.CLICK_BUY_COMPASS.format("price", UtilityMethods.format(BountyHunters.getInstance().getConfig().getDouble("player-tracking.price"))));
        ...
        if (page < maxPage) inv.setItem(26, CustomItem.NEXT_PAGE.toItemStack());
        if (page > 1) inv.setItem(18, CustomItem.PREVIOUS_PAGE.toItemStack());
    ...
    public void whenClicked(ItemStack item, InventoryAction action, int slot) {
        ...
        if (!UtilityMethods.hasItemMeta(item, false)) return;
        ...
        if (action == InventoryAction.PICKUP_ALL && BountyHunters.getInstance().getConfig().getBoolean("player-tracking.enabled")) {
            ...
            if (bounty.hasHunter(player)) {
                ...
                if (bounty.hasTarget(player)) return;

                // Check for tax
                double taxed = BountyHunters.getInstance().getConfigManager().targetSetTax.calculate(bounty.getReward(), false);
                ...
                if (hunterEvent.isCancelled()) return;

                BountyHunters.getInstance().getEconomy().withdrawPlayer(player, taxed);
                data.setLastTarget();
                bounty.addHunter(player);
                if (target.isOnline()) hunterEvent.sendAllert(target.getPlayer());
        ...
        if (action == InventoryAction.PICKUP_HALF) {
            if (!bounty.hasContributed(player) || !player.hasPermission("bountyhunters.remove")) return;
            ...
            final double taxed = BountyHunters.getInstance().getConfigManager().bountyRemovalTax.calculate(bountyEvent.getAmountRemoved(), true);
            BountyHunters.getInstance().getEconomy().depositPlayer(player, bountyEvent.getAmountRemoved() - taxed);

            // Unregister bounty if needed
            if (bountyEvent.isExpiring()) BountyHunters.getInstance().getBountyManager().unregisterBounty(bounty, true);
