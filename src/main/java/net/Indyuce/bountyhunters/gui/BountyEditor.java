...
public class BountyEditor extends PluginInventory {
    ...
    public Inventory getInventory() {
        ...
        bookLore.add("");
        bookLore.add(ChatColor.GRAY + "Reward: " + ChatColor.GOLD + "$" + UtilityMethods.format(reward));
        bookLore.add(ChatColor.GRAY + "  - From Players: " + ChatColor.GOLD + "$" + UtilityMethods.format(reward - bounty.getExtra()));
        bookLore.add(ChatColor.GRAY + "  - Extra: " + ChatColor.GOLD + "$" + UtilityMethods.format(bounty.getExtra()));
        bookLore.add("");
        ...
        chestLore.add("");
        chestLore.add(ChatColor.GRAY + "Amount: " + ChatColor.GOLD + "$" + UtilityMethods.format(bounty.getExtra()));
        chestLore.add("");
        ...
        if (bounty.getContributors().size() == 0) {
            ...
            for (int j = 0; j < slots.length; j++) {
                ...
                lore.add("");
                lore.add(ChatColor.GRAY + "Amount: " + ChatColor.GOLD + "$" + UtilityMethods.format(contribution));
    ...
    public void whenClicked(ItemStack item, InventoryAction action, int slot) {
        ...
        if (!UtilityMethods.hasItemMeta(item, false))
            return;
        ...
        if (item.getItemMeta().getDisplayName().equals(ChatColor.GREEN + "Previous")) {
            ...
            new ChatInput(this, input -> {
                try {
                    ...
                    player.sendMessage(ChatColor.GREEN + "> Extra reward successfully set to $" + UtilityMethods.format(d) + ".");
                    player.playSound(player.getLocation(), Sound.BLOCK_NOTE_BLOCK_PLING, 1, 2);
                    bounty.setExtra(d);
                    return true;
