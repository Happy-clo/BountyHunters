...
public class BountiesCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args[0].equalsIgnoreCase("titles")) {
            ...
            for (HunterTitle title : BountyHunters.getInstance().getLevelManager().getTitles())
                if (playerData.hasUnlocked(title))
                    ((Player) sender).spigot().sendMessage(ChatMessageType.CHAT,
                            new ComponentBuilder()
                                    .append("- " + title.format())
                                    .event(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/bounties title " + title.getId()))
                                    .event(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text(Language.CLICK_SELECT.format())))
            ...
            for (BountyAnimation anim : BountyHunters.getInstance().getLevelManager().getAnimations())
                if (playerData.hasUnlocked(anim))
                    ((Player) sender).spigot().sendMessage(ChatMessageType.CHAT,
                            new ComponentBuilder()
                                    .append("- " + anim.format())
                                    .event(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/bounties animation " + anim.getId()))
                                    .event(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text(Language.CLICK_SELECT.format())))
                                    .color(net.md_5.bungee.api.ChatColor.GREEN)
                                    .create());
        }
        ...
        if (args[0].equalsIgnoreCase("reload")) {
            ...
            BountyHunters.getInstance().getConfigManager().reload();
