...
public class YAMLPlayerDataManager extends PlayerDataManager {
	...
	public void loadData(PlayerData data) {
		...
		if (config.contains("current-anim"))
			try {
				Validate.isTrue(levelManager.hasAnimation(config.getString("current-anim")), "Could not load animation from " + data.getPlayerName());
		...
		if (config.contains("current-title"))
			try {
				Validate.isTrue(levelManager.hasTitle(config.getString("current-title")), "Could not load title from " + data.getPlayerName());
