...
public class Bounty {
    ...
    private final Map<OfflinePlayer, Double> amount = new LinkedHashMap<>();
     ...
     */
    public Bounty(@Nullable OfflinePlayer creator, @NotNull OfflinePlayer target, double reward) {
        Validate.notNull(target, "Target cannot be null");
        Validate.notNull(target.getName(), "Target cannot be null");

        id = UUID.randomUUID();
        this.target = target;
        if (creator != null) amount.put(creator, reward);
        else extra += reward;
    }
     ...
     * @param reward Bounty base reward. Can be increased later
     */
    public Bounty(@NotNull OfflinePlayer target, double reward) {
     ...
     * @param reward Bounty base reward. Can be increased later
     */
    public Bounty(@NotNull UUID id, @NotNull OfflinePlayer target, double reward) {
    ...
    /**
     * @return Time in millis before the bounty expires. Result is nonsense
     * if the inactivity bounty removal option is toggled off.
     */
    public long getExpireDelay() {
        return Math.max(0, lastUpdated + BountyHunters.getInstance().getConfigManager().bountyInactivityTime - System.currentTimeMillis());
    }
    ...
    public void removeHunter(OfflinePlayer player) {
        Validate.isTrue(hasHunter(player), "Player is not hunting this bounty");
