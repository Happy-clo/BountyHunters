...
public class BountyClaimEvent extends BountyEvent {
	...
	public void sendAllert() {
		...
		// message to player
		String reward = UtilityMethods.format(getBounty().getReward());
		...
		for (Player online : Bukkit.getOnlinePlayers())
			if (online != player)
				Message.BOUNTY_CLAIMED.format("reward", UtilityMethods.format(getBounty().getReward()), "killer", title + player.getName(),
