...
public enum ParticleEffectType {
            ...
            public void run() {
                ...
                for (int k = 0; k < 2; k++)
                    for (int j = 0; j < 2; j++) {
                        ...
                        if (UtilityMethods.RANDOM.nextDouble() < .05)
    ...
    GOLD((loc, target) -> {
        ...
        for (int j = 0; j < 16; j++) {
            ...
            double r = .7 + UtilityMethods.RANDOM.nextDouble() * .3;
            double a = UtilityMethods.RANDOM.nextDouble() * Math.PI * 2;
            Vector vec = new Vector(Math.cos(a) * r, 2, Math.sin(a) * r);
            item.setVelocity(vec.multiply(.15));

            Bukkit.getScheduler().runTaskLater(BountyHunters.getInstance(), () -> item.remove(), 50 + UtilityMethods.RANDOM.nextInt(50));
        }
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (int j = 0; j < 6; j++) {
                    ...
                    if (UtilityMethods.RANDOM.nextDouble() < .07)
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (int k = 0; k < 15; k++) {
                    ...
                    if (UtilityMethods.RANDOM.nextDouble() < .2)
    ...
    METEOR((loc, target) -> {
        ...
        for (int k = 0; k < 3; k++)
            new BukkitRunnable() {
                int ti = 0;
                Location source = loc.clone().add(5 * Math.cos(UtilityMethods.RANDOM.nextDouble() * 2 * Math.PI), 20, 5 * Math.sin(UtilityMethods.RANDOM.nextDouble() * 2 * Math.PI));
                ...
                public void run() {
                    ...
                    for (int k = 0; k < 2; k++) {
                        ...
                        for (double i = 0; i < Math.PI * 2; i += Math.PI / 6) {
                            Vector vec = UtilityMethods.rotateFunc(new Vector(Math.cos(i), Math.sin(i), 0), source);
